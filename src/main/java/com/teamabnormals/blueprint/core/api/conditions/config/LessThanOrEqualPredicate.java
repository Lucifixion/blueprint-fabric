...
/**
 * A config predicate that checks whether the number got from a {@link ForgeConfigSpec.ConfigValue} instance is less than or equal to a stored number.
...
public class LessThanOrEqualPredicate implements IConfigPredicate {
	private static final ResourceLocation ID = new ResourceLocation(BlueprintForge.MOD_ID, "less_than_or_equal_to");
	...
	public static class Serializer implements IConfigPredicateSerializer<LessThanOrEqualPredicate> {
		...
		public void write(JsonObject json, IConfigPredicate value) {
			if (!(value instanceof LessThanOrEqualPredicate))
